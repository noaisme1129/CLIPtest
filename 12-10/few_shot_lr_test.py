...
learning_rates = [1e-2, 3.16e-3, 1e-3, 3.16e-4, 1e-4, 3.16e-5, 1e-5, 3.16e-6, 1e-6, 3.16e-7, 1e-7, 3.16e-8, 1e-8]
...
for lr in learning_rates:
    ...
    # Freeze all layers except text embeddings
    for param in model.parameters():
        param.requires_grad = False
    for param in model.token_embedding.parameters():
        param.requires_grad = True
    ...
    # Print validation and test accuracy for the current learning rate
    print(f"Learning rate: {lr}, Validation Accuracy: {val_accuracy:.4f}, Test Accuracy: {test_accuracy:.4f}")

# Plotting validation and test accuracies
plt.figure(figsize=(10, 6))
plt.plot(learning_rates, val_accuracies, label="Validation Accuracy", marker='o')
plt.plot(learning_rates, test_accuracies, label="Test Accuracy", marker='s')
plt.xscale("log")
plt.xlabel("Learning Rate")
plt.ylabel("Accuracy")
plt.title("Validation and Test Accuracy vs Learning Rate for 1-shot")
plt.legend()
